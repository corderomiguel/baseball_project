...
Usando el comando `hist` podemos obtener el histograma de la variables y la descripción de los puntos de corte de cada frecuencia; estos serán de particular interes.

```{r echo=FALSE, fig.asp=0.6,fig.width=6, paged.print=TRUE}
library(stringr)
hist(X1,main = "Histograma variable X1", ylab = "Frecuencia", xlab = "X1: Hits al bate", col="Yellow")
(objeto<- hist(X1, plot=F) )
pc <- objeto$breaks
fr <- objeto$counts
limites<-c(rep(0,10))
for (i in 1:10) {
  limites[i]<- str_c(c(pc[i],pc[i+1]), sep="- ")
}

(pc_y_fr <- rbind(limites,fr))
```
